...
The data are divided into 70% train data and 30% test data because there are 2930 observations, which are large enough for that ratio.
...
To choose the best linear model, we fit several models on the train data and check the errors on the test data. At the end, we chose the best result for linear model. 

```{r lm-model-1}
# Simple model based on stepwise variables
lm1_train <- lm(log(SalePrice) ~ Gr.Liv.Area + Year.Built + 
                  Overall.Qual + Overall.Cond + Total.Bsmt.SF, data = d_train)
...
# RMSE
## Way 1: manually
sqrt(mean((exp(pred_lm2_test) - d_test$SalePrice)^2))
...
# RMSE
## Way 1: manually
sqrt(mean((exp(pred_lm3_test) - d_test$SalePrice)^2))
...
```{r lm-model-4}
# Simple model based on stepwise variables
lm4_train <- lm(log(SalePrice) ~ Gr.Liv.Area +Neighborhood +
...
# RMSE
## Way 1: manually
sqrt(mean((exp(pred_lm4_test) - d_test$SalePrice)^2))
