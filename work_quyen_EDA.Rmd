...
lm_final_cross1 <-lm(log(SalePrice) ~ Gr.Liv.Area + Overall.Qual + Overall.Cond + 
                  Year.Built + Total.Bsmt.SF + Fireplaces + Garage.Cars + 
                  MS.Zoning + Bldg.Type + Functional, 
                  data = d_house[split$train, ,!colnames(train) %in% c("MS.Zoning")])
...
For a given home, increasing its number of Garage area by one square feet, would results in about 0.15% more cars
For a given home, increasing its SalePrice by 10000 USD, would results in about 3% more cars


```{r}
# Fit the poisson model to predict number of garage cars
glm1_poisson <- glm(Fireplaces ~ Garage.Area + SalePrice + Land.Contour, 
                   family = "poisson", ## we specify the distribution!
                   data = d_house)
