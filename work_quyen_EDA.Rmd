...
The data are divided into 70% train data and 30% test data because there are 2930 observations, which are large enough for that ratio. From the scatter plots in the numerical analysis part, there are some outliers, which do not follow the trend in the Gr.Liv.Area and Total.Bsmt.SF. Thus, we remove them first before fitting the model because keeping them will make the errors become extremely high and there are only 3 outliers. 
...
d_house <- d_house %>%
  filter(Gr.Liv.Area < 4500 & Total.Bsmt.SF < 4500 & Garage.Cars < 5)
...
pred_lm1_test <- predict(lm1_train, newdata = d_lm_test)

# R^2
r_squared_lm1 <- cor(exp(pred_lm1_test), d_lm_test$SalePrice)^2
# RMSE
## Way 1: manually
sqrt(mean((exp(pred_lm1_test) - d_lm_test$SalePrice)^2))
## Way 2: by function
accuracy(exp(pred_lm1_test), d_lm_test$SalePrice)
...
Linear model 5 appears as the best one. 
...
```{r lm-model-9}
# Model with variables appearing in several analysis
lm9_train <- lm(log(SalePrice) ~ Gr.Liv.Area + Overall.Qual + Year.Built +
                  Overall.Cond + MS.Zoning + Sale.Condition, 
                data = d_train)
# Predict the model 2 on test data
